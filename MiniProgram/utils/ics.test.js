...
const semesterOutput = generateCalendar([regular], table, periods, { reminderMinutesOverride: 10, now: '2026-08-27T00:00:00Z' })
assert.match(semesterOutput, /TRIGGER:-PT10M/)
const alarmOutput = generateCalendar([regular], table, periods, { reminderMinutesOverride: 5, reminderTypeOverride: 'alarm', now: '2026-08-27T00:00:00Z' })
assert.match(alarmOutput, /ACTION:AUDIO/)
assert.doesNotMatch(alarmOutput, /ACTION:DISPLAY/)
assert.match(output, /X-WR-TIMEZONE:Asia\/Shanghai/)
...
assert.throws(() => generateCalendar([regular], { ...table, semesterStart: '2026-02-31' }, periods), /第一周日期/)
