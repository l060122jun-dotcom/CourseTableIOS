...
Page({
  ...
  chooseImage() { wx.chooseMedia({ count: 1, mediaType: ['image'], sourceType: ['album', 'camera'], success: result => { const path = result.tempFiles[0].tempFilePath; this.setData({ message: '图片已选择，正在识别…', draft: null }); ocrAdapter.recognize(path).then(draft => { const courses = (draft.courses || []).map((course, index) => ({ ...course, color: course.color || courseColor(course.name), id: course.id || 'ocr-' + Date.now() + '-' + index })); this.setData({ draft, previewCourses: courses, message: '' }) }).catch(error => this.setData({ message: error.message || '识别失败' })) }, fail: error => this.setData({ message: '未选择图片：' + error.errMsg }) }) },
