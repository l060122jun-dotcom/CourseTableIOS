...
function courseColor(name) { const text = String(name || ''); let hash = 0; for (let i = 0; i < text.length; i += 1) hash = (hash * 31 + text.charCodeAt(i)) >>> 0; return palette[hash % palette.length] }
Page({
  data: { message: '', draft: null, previewCourses: [], periods: [], days: ['一', '二', '三', '四', '五', '六', '日'] },
  onShow() { this.setData({ periods: store.load().periods || [] }) },
  chooseImage() { wx.chooseMedia({ count: 1, mediaType: ['image'], sourceType: ['album', 'camera'], success: result => { const path = result.tempFiles[0].tempFilePath; this.setData({ message: '图片已选择，正在识别…', draft: null }); ocrAdapter.recognize(path).then(draft => { const courses = (draft.courses || []).map((course, index) => ({ ...course, color: course.color || courseColor(course.name), spanHeight: course.spanHeight || ((course.endPeriod || course.startPeriod || 1) - (course.startPeriod || 1) + 1) * 190 - 8, id: course.id || 'ocr-' + Date.now() + '-' + index })); this.setData({ draft, previewCourses: courses, message: '' }) }).catch(error => this.setData({ message: error.message || '识别失败' })) }, fail: error => this.setData({ message: '未选择图片：' + error.errMsg }) }) },
